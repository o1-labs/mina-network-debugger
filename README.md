...
11. [The Network Debugger Front End](#the-network-debugger-front-end)
...
This installs: git, curl, libelf-dev, protobuf-compiler, clang, libssl-dev, pkg-config, libbpf-dev, make, libbz2-dev

### Full Development Setup

Install all development dependencies (Rust, BPF linker, Cap'n Proto):
...
```bash
make setup-rust        # Install Rust and components (rustfmt, clippy, rust-src)
make setup-bpf-linker  # Install BPF linker for eBPF compilation
make setup-capnproto   # Install Cap'n Proto compiler
```
...
```bash
make build-kern          # BPF kernel module only
make build-aggregator    # Aggregator binary
make build-topology-tool # Topology tool binary
```

> **Note:** Cargo will display a warning about `main.rs` being present in multiple build targets. This is intentional.
...
This builds the release binary and runs it with sudo. The debugger requires root privileges.
...
### Connecting a Mina Application

In a separate terminal, run the Mina application with the `BPF_ALIAS` environment variable set.

> **Important:** Set this variable for the Mina application, not for the debugger.

The value format is: `${CHAIN_ID}-${EXTERNAL_IP}`

Where `${CHAIN_ID}` is one of:
- `mainnet`
- `devnet`
- `berkeley`
- A literal chain ID like `/coda/0.0.1/d8a8e53385b4629a1838156529ff2687e31f951873704ddcc490076052698a88`
...
make test-release  # Release mode
```

### Integration Test

The integration test opens TCP connections with itself and simultaneously performs disk IO, verifying the debugger sees only TCP data correctly.
...
The debugger will print "test is passed" in the log when successful.
...
make docker-build-commit  # Tag with git commit hash
```
...
This repository contains `run.yaml` specification, which runs the latest debugger and Mina node.
...
Environment variables for the debugger:

| Variable | Default | Description |
|----------|---------|-------------|
| `SERVER_PORT` | `8000` | HTTP server port |
| `DB_PATH` | `target/db` | RocksDB database path |
| `DRY` | (unset) | Set any value to disable BPF (useful for DB inspection) |
| `HTTPS_KEY_PATH` | (unset) | Path to TLS private key |
| `HTTPS_CERT_PATH` | (unset) | Path to TLS certificate |
| `DEBUGGER_INDEX_LEDGER_HASH` | (unset) | Enable ledger hash indexing (CPU intensive) |
| `FIREWALL_INTERFACE` | `eth0` | Network interface for firewall attachment |

> **Note:** The log line `libbpf: BTF loading error: -22` may be ignored. It occurs because the BPF module is written in Rust, which generates incompatible debug information.
...
Mina p2p traffic conforms to this protocol stack (incomplete):

* [Private Networks](https://github.com/libp2p/specs/blob/0c40ec885645c13f1ed43f763926973835178c6e/pnet/Private-Networks-PSK-V1.md). Uses XSalsa20 stream with pre-shared key. The key is derived from mina configuration, so it is not really a secret key, but known for every peer that has the same config.
* [Connection establishment](https://github.com/libp2p/specs/tree/0c40ec885645c13f1ed43f763926973835178c6e/connections).
* [Multistream Select](https://github.com/multiformats/multistream-select/tree/c05dd722fc3d53e0de4576161e46eea72286eef3). Negotiate all further protocols. Mina usually uses `/noise` and may use `/libp2p/simultaneous-connect`.
...
Click on **Expand all** to show full details of all values, and **Collapse all** to minimize them. You can **Copy** the information into your clipboard or **Save** it, either as a **JSON** file or as a **Bin**. You can also click on **Copy link** to copy a web link for this message to your clipboard.
...
Same as with the Message Hex, you can **Copy** the information into your clipboard or **Save** it, either as a JSON file or as a Bin. Click on **Copy link** to copy the link to the message to your clipboard.
...
**FD** - the message's TCP socket ID (a file descriptor, but it is used for items other than files). The fd is valid inside the process, another process may also have the same fd, but it is a different socket. Similarly to pid, it is subject to change when the connection is closed or fails.
